...
static bool runInstructionObfuscation(Function &F) {
  // Skip declarations
  if (F.isDeclaration()) {
    return false;
  }
...
struct InstructionObfuscationPass : public PassInfoMixin<InstructionObfuscationPass> {
  PreservedAnalyses run(Function &F, FunctionAnalysisManager &FAM) {
    if (runInstructionObfuscation(F))
...
#else
struct InstructionObfuscationPass : public FunctionPass {
  static char ID;
  InstructionObfuscationPass() : FunctionPass(ID) {}

  bool runOnFunction(Function &F) override {
    return runInstructionObfuscation(F);
  }
...
#else
char InstructionObfuscationPass::ID = 0;
static RegisterPass<InstructionObfuscationPass>
X("kovid-instruction-obf", "KoviD Instruction Obfuscation Pass");
#endif
